...
interface PostMessageMultiThreadProps {
  formState: IFormState;
  threads: number;
}

const PostMessageMultiThread: React.FC<PostMessageMultiThreadProps> = ({ formState, threads }) => {
  ...
  const onClick = () => {
    ...
    const workers = Array.from({length: threads}, () => (
      new Worker(new URL("../workers/ArrayWorker.ts", import.meta.url))
    ));
  ...
  return (
    <div>
      <Button type="submit" id="postMessage2threadBtn" onClick={onClick}>{`Вычислить с postMessage (Потоков ${threads})`}</Button>
      {
        isLoading
...
export default PostMessageMultiThread;
